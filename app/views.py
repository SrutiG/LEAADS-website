...
def signup():
    if (request.method == 'POST'):
        ...
        conn = mysql.connection;
        cursor = conn.cursor();
        cursor.execute("UPDATE USER SET NAME = %s, AGE = %s, DESCRIPT = %s, ADDRESS = %s, COURT = %s ,GENDER = %s, NUMBER = %s WHERE USERNAME = user", (name, age, descript, address, court, gender, number));
        conn.commit
        session['login'] = True;
        session['user'] = username;
        return json.dumps({'success':True}), 200;
